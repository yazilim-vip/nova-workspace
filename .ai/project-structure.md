...
Cross-project concerns — safety rules, repo maps, infra conventions, and skills that apply to *every* repo — belong to the **workspace**, not the project. If you need to coordinate across multiple repos, that's a workspace (see `.ai/procedures/onboarding/`), not a "multi-repo project." No nested workspaces.
...
- Workspace-level concerns (cross-project safety rules, repo map, framework procedures)
