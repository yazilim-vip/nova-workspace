...
description: Manages the user's personal knowledge vault under `notes/` — vault setup, capture into PARA folders (inbox, daily, projects, areas, resources, archive), retrieval, organization. Covers all read/write activity inside `notes/`. For viewer-specific setup (Obsidian, Foam, etc.) the procedure delegates to `.ai/procedures/personal-knowledge-management/adapters/<viewer>/README.md`.
---

# Notes

Framework procedure — manages the user's personal knowledge vault: capture, organize, search. Vault is gitignored under `notes/`; viewer-specific overrides live in `adapters/<viewer>/`.
...
- The vault is plain markdown + YAML frontmatter — readable in any editor. **Viewer choice is opt-in** and lives under `.ai/procedures/personal-knowledge-management/adapters/<viewer>/`. See § Viewers below.
...
If a rule applies to every NOVA agent, it belongs in root `AGENTS.md`. If it applies to every notes vault, it belongs here. Viewer-specific config (Obsidian plugins, Foam settings, etc.) lives under `.ai/procedures/personal-knowledge-management/adapters/<viewer>/` — never in this file. Agent-specific shell-command JSON or CLI invocations live under `.ai/procedures/adapters/<agent>/`.
...
For viewer setup ("set up Obsidian for my notes"), the agent reads the matching adapter under `.ai/procedures/personal-knowledge-management/adapters/<viewer>/`.
...
Viewer support is opt-in and isolated under `.ai/procedures/personal-knowledge-management/adapters/<viewer>/`, mirroring the per-platform pattern of `.ai/procedures/adapters/<coding-agent>/`. The vault itself is plain markdown and works without any viewer.

| Viewer | Adapter | Status |
|--------|---------|--------|
| Obsidian | `.ai/procedures/personal-knowledge-management/adapters/obsidian/` | supported |
...
See `.ai/procedures/personal-knowledge-management/adapters/README.md` for the index.
...
- **Don't put viewer-specific rules in this file.** They go under `.ai/procedures/personal-knowledge-management/adapters/<viewer>/`.
...
- `.ai/procedures/adapters/<agent>/` — where agent-specific shell-command JSON belongs if a workspace pre-bakes it (otherwise the agent generates on demand).
