...
Per-viewer implementations of the notes vault contract (`.ai/procedures/personal-knowledge-management/PROCEDURE.md`). Same shape as `.ai/procedures/adapters/<coding-agent>/`: the contract is tool-neutral; adapters are opt-in per-viewer specifics.
...
If a rule applies to every notes vault, it belongs in `.ai/procedures/personal-knowledge-management/PROCEDURE.md`. If it applies only to one viewer, it belongs under `.ai/procedures/personal-knowledge-management/adapters/<viewer>/`. Adapters reference the contract; they never restate or contradict it.
...
1. Create `.ai/procedures/personal-knowledge-management/adapters/<viewer>/README.md`.
