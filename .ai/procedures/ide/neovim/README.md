...
Procedure — read when the user asks to set up Neovim as a terminal-native IDE wired to Claude Code over MCP. Peer of `.ai/procedures/ide/intellij/`.
...
- `.ai/procedures/terminal/tmux/README.md` — multiplexer layer. Optional. If present, `vim-tmux-navigator` makes `Ctrl+h/j/k/l` cross nvim/tmux pane boundaries.
- `.ai/procedures/terminal/PROCEDURE.md` § "Terminal emulator requirements (for Claude Code)" — host-emulator settings (`Shift+Enter`, meta-as-alt). Required regardless of multiplexer.
