...
|-------|-----------|----------------|
| **Framework procedure change** | Change to a `.ai/procedures/<name>/PROCEDURE.md` (e.g. `onboarding`, `self-update`) or its supporting files | Usually apply — core machinery. If the fork customized the procedure, surface the diff. |
| **Convention doc change** | Change to flat `.ai/*.md` (e.g. `.ai/project-structure.md`) | Apply unless the fork has diverged |
| **Onboarding template change** | `.ai/procedures/onboarding/assets/*` | Apply unless the fork has diverged |
