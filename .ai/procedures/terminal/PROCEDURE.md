...
- tmux → read `.ai/procedures/terminal/tmux/README.md` and follow its install steps.
...
Out of scope for this skill. Hand off to `ide` (specifically `.ai/procedures/ide/neovim/README.md` for the most common Claude-friendly editor stack).
...
- **Editor**: defer to `ide`'s verification.
...
|-------------|--------------|--------|
| tmux        | `.ai/procedures/terminal/tmux/` | supported |
...
2. **Multiplexer** — `.ai/procedures/terminal/tmux/README.md` (includes the opt-in one-shot bindings block: Alt+i/j/k/l pane focus, Alt+Left/Right windows, etc.).
3. **Editor** — `.ai/procedures/ide/neovim/README.md` (`claudecode.nvim` MCP bridge).
...
- Multiplexer-specific rules live under `.ai/procedures/terminal/<multiplexer>/` — never copied into this file.
- Editor-specific rules live under `.ai/procedures/ide/<editor>/` — never duplicated here.
