...
Implements `.ai/enforcement.md` for [Claude Code](https://code.claude.com). Generates `.claude/` at the workspace root — gitignored, per-developer, regenerable by the adapters procedure (`.ai/procedures/adapters/PROCEDURE.md`).
...
Hooks `cat` a shared file. They do not paraphrase. If future work tempts you to inline a rule into a hook script, re-read `.ai/procedures/adapters/PROCEDURE.md` — the adapter is a pointer, not a copy. Same rule binds hooks.
...
**When to invoke.** User-triggered only — via `/dream` slash command, or explicit "use the dream-worker subagent". Auto-invocation is disabled on the command side so Claude doesn't proactively tidy the workspace. Full procedure at `.ai/procedures/dream/PROCEDURE.md`.
...
- **Framework procedures (`nova-*`)** — stay at `.ai/procedures/<name>/PROCEDURE.md` (committed). Load via the root `AGENTS.md` "Framework Procedures" table — that table is `@`-imported into context at session start and the agent reasons against each skill's `description` to route.
...
Shortcut that delegates to the `dream-worker` subagent. Uses `disable-model-invocation: true` — Claude will not invoke `/dream` on its own; only the user can trigger it. MVP is strictly user-triggered; scheduled/automatic dreaming is explicitly deferred per `.ai/procedures/dream/PROCEDURE.md`.
...
Runtime outputs (the workspace-root `.claude/` only) are gitignored. Sources under `.ai/procedures/adapters/claude/` are committed. The procedure does NOT write anywhere under `git-repositories/<repo>/` — see `.ai/procedures/adapters/PROCEDURE.md` step 7 hard rule.
