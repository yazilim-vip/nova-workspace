...
**Generation.** This file is the source of truth. The adapters procedure (`.ai/procedures/adapters/PROCEDURE.md` step 5) extracts the single fenced ` ```bash ``` ` block below, writes it to the runtime path, and `chmod +x`. Edit this file — never the runtime copy.
