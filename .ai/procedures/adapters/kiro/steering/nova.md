...
`AGENTS.md` is the entry point. Workspace overrides at `.ai/workspace/AGENTS.md` (if missing → run `.ai/procedures/onboarding/PROCEDURE.md`). Repo map at `.ai/workspace/map/repos.md` — never guess paths; read it when the task names a repo. Per-repo conventions at each `git-repositories/<repo>/AGENTS.md`. Skills, learnings, procedures elsewhere under `.ai/`. Read what the task demands; don't preload.

## Strict platform rules (read before shell use)

#[[file:.ai/procedures/adapters/kiro/terminal.md]]

## Strict steering discipline (read before editing any `.kiro/steering/` file)

#[[file:.ai/procedures/adapters/kiro/steering-discipline.md]]

## Personal-knowledge-management — agent doctrine (always-on)

#[[file:.ai/procedures/adapters/_shared/personal-knowledge-management.md]]
