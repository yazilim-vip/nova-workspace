...
#[[file:.ai/procedures/dream/PROCEDURE.md]]
...
Your job: produce a **Dream Report** per the format in `.ai/procedures/dream/PROCEDURE.md`. The user approves proposals; the parent agent applies them.

## First actions, every invocation

1. **Re-read `.ai/procedures/dream/PROCEDURE.md`** to confirm the five review categories (Dedupe, Promote, Demote, Compact, Audit). Do not invent new categories.
...
4. **Run the five reviews** per the signals in `.ai/procedures/dream/PROCEDURE.md`. Be strict; don't manufacture proposals.
...
Before any shell command would be needed, stop — you have no shell tools, and don't need them. If you feel like running `ls` or `cat`, use `glob` and `read` instead. Terminal rules in `.ai/procedures/adapters/kiro/terminal.md` apply regardless; the safest path is using only the tools you have.

## Output

Return the Dream Report verbatim in the format specified by `.ai/procedures/dream/PROCEDURE.md`. No preamble. No caveats. The report is the deliverable.
