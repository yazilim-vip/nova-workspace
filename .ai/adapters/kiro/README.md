...
| **C2** Per-turn re-injection | Hook with `promptSubmit` trigger, shell-command action. Prints the shared checklist + matches `$USER_PROMPT` against `.ai/adapters/_shared/task-pointers.md` and emits matched pointers as "Consider:" lines (parity with Claude's `user-prompt-submit` hook). | `hooks/prompt-submit.md` (bash extracted at install) + `hooks/prompt-submit.kiro.hook` | `.kiro/hooks/...` |
