...
| **C4** Host-environment doctrine | `@`-imported strict rules; activation gated on the workspace's declared `Host Environments` (see `.ai/workspace/AGENTS.md` and `.ai/onboarding/README.md` step 6). Each host gets one doctrine file. | `intellij-mcp.md` (more as hosts are added) | Imported by `steering/CLAUDE.md` → `.claude/CLAUDE.md` |
