...
1. **Re-read `.ai/nova-skills/dream/SKILL.md`** — it's in your context via `@` import above, but re-read specifically to confirm the five review categories (Dedupe, Promote, Demote, Compact, Audit). Do not invent new categories.
...
4. **Run the five reviews.** Apply the signals defined in `.ai/nova-skills/dream/SKILL.md`. Be strict about what counts as a signal; don't manufacture proposals.
...
Return the Dream Report verbatim in the format specified by `.ai/nova-skills/dream/SKILL.md`. No preamble, no caveats, no "I hope this helps." The report is the deliverable; the parent reads it and acts.
