...
> **Anti-duplication.** This file owns *how the agent recognizes and routes PKM intent*. Vault layout, frontmatter, capture-verb behavior, and viewer-specific rules live in `.ai/nova-skills/personal-knowledge-management/` — never restated here.
...
1. **Read the contract:** `.ai/nova-skills/personal-knowledge-management/SKILL.md`. It owns vault layout (PARA), naming, frontmatter schema, folder-notes layout, capture vocabulary behaviors. Do not infer these — read them.
2. **Detect viewer:** check whether `notes/.obsidian/` exists at the workspace root (`Bash` `test -d notes/.obsidian` or equivalent). If yes, also read `.ai/nova-skills/personal-knowledge-management/adapters/obsidian/README.md` — its **strong rule "every note is a folder note"** overrides the contract's flat-by-default preference.
...
| `.ai/nova-skills/personal-knowledge-management/` | Read for context. Edit only when the user asks to evolve the contract or an adapter — that's a framework change, not PKM use. |
...
- `.ai/nova-skills/personal-knowledge-management/SKILL.md` — vault contract.
- `.ai/nova-skills/personal-knowledge-management/adapters/<viewer>/README.md` — viewer-specific overrides.
