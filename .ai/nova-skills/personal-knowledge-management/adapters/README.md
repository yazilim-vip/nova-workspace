...
Per-viewer implementations of the notes vault contract (`.ai/nova-skills/personal-knowledge-management/SKILL.md`). Same shape as `.ai/nova-skills/adapters/<coding-agent>/`: the contract is tool-neutral; adapters are opt-in per-viewer specifics.
...
If a rule applies to every notes vault, it belongs in `.ai/nova-skills/personal-knowledge-management/SKILL.md`. If it applies only to one viewer, it belongs under `.ai/nova-skills/personal-knowledge-management/adapters/<viewer>/`. Adapters reference the contract; they never restate or contradict it.
...
1. Create `.ai/nova-skills/personal-knowledge-management/adapters/<viewer>/README.md`.
