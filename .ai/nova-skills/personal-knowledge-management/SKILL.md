...
- The vault is plain markdown + YAML frontmatter — readable in any editor. **Viewer choice is opt-in** and lives under `.ai/nova-skills/personal-knowledge-management/adapters/<viewer>/`. See § Viewers below.
...
If a rule applies to every NOVA agent, it belongs in root `AGENTS.md`. If it applies to every notes vault, it belongs here. Viewer-specific config (Obsidian plugins, Foam settings, etc.) lives under `.ai/nova-skills/personal-knowledge-management/adapters/<viewer>/` — never in this file. Agent-specific shell-command JSON or CLI invocations live under `.ai/nova-skills/adapters/<agent>/`.
...
For viewer setup ("set up Obsidian for my notes"), the agent reads the matching adapter under `.ai/nova-skills/personal-knowledge-management/adapters/<viewer>/`.
...
Viewer support is opt-in and isolated under `.ai/nova-skills/personal-knowledge-management/adapters/<viewer>/`, mirroring the per-platform pattern of `.ai/nova-skills/adapters/<coding-agent>/`. The vault itself is plain markdown and works without any viewer.

| Viewer | Adapter | Status |
|--------|---------|--------|
| Obsidian | `.ai/nova-skills/personal-knowledge-management/adapters/obsidian/` | supported |
...
See `.ai/nova-skills/personal-knowledge-management/adapters/README.md` for the index.
...
- **Don't put viewer-specific rules in this file.** They go under `.ai/nova-skills/personal-knowledge-management/adapters/<viewer>/`.
...
- `.ai/nova-skills/personal-knowledge-management/adapters/README.md` — viewer adapter index.
- `.ai/workspace/learnings/` — agent-consumed cross-project knowledge (different audience).
- `.ai/nova-skills/adapters/<agent>/` — where agent-specific shell-command JSON belongs if a workspace pre-bakes it (otherwise the agent generates on demand).
