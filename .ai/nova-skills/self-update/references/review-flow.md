...
|-------|-----------|----------------|
| **Framework skill change** | Change to a `.ai/nova-skills/<name>/SKILL.md` (e.g. `nova-onboarding`, `nova-self-update`) or its supporting files | Usually apply — core machinery. If the fork customized the skill, surface the diff. |
| **Convention doc change** | Change to flat `.ai/*.md` (e.g. `.ai/project-structure.md`) | Apply unless the fork has diverged |
| **Onboarding template change** | `.ai/nova-skills/onboarding/assets/*` | Apply unless the fork has diverged |
