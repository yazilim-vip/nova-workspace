...
Framework skill — generates IDE project configuration so cloned repos under `git-repositories/` open as native modules. Per-IDE implementation lives under `.ai/nova-skills/ide/<ide>/`.
...
**Committed procedure, per-dev artifacts** — same model as `.ai/nova-skills/adapters/`:

- The procedure docs + templates live under `.ai/nova-skills/ide/<platform>/` — tracked, team-shared.
...
|----------|-------------|--------|-------------------|
| IntelliJ IDEA | `.ai/nova-skills/ide/intellij/` | `.idea/` at workspace root | `modules.xml`, `.iml` per repo, `vcs.xml`, `misc.xml`, `encodings.xml`, `runConfigurations/` |
| Neovim | `.ai/nova-skills/ide/neovim/` | _(none — configs live in `~/.config/nvim/`)_ | Install recipe for `vim.pack` + `claudecode.nvim` + `snacks.nvim` + fzf-lua/yazi/lazygit. Composes with `.ai/nova-skills/terminal/tmux/`. |
